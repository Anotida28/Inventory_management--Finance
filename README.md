...
- **frontend/**: Next.js application with route wrappers in `src/app`, route-level screen modules in `src/screens` (the closest safe equivalent to `src/pages` in a Next app), shared UI in `src/components`, and API/store code in `src/services`.
- **backend/**: Express/TypeScript API with routes, controllers, Prisma files, and local seed-backed development data.
