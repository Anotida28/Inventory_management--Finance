...
export type UploadedIssueAttachment = {
  originalName: string;
  storedName: string;
  storagePath: string;
  mimeType: string;
...
const configuredUploadPath =
  process.env.OPERATIONS_UPLOAD_PATH || "./data/uploads/issues";
...
const configuredReceiptUploadPath =
  process.env.RECEIPT_UPLOAD_PATH || "./data/uploads/receipts";
const resolvedReceiptUploadRoot = path.isAbsolute(configuredReceiptUploadPath)
  ? configuredReceiptUploadPath
  : path.join(process.cwd(), configuredReceiptUploadPath);
...
const isAllowedUpload = (file: { originalname: string; mimetype: string }) => {
...
export const runIssueAttachmentUpload = (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const mapUploadedIssueFiles = (
...
export const removeUploadedIssueFiles = (files: Express.Multer.File[]) => {
  files.forEach((file) => {
...
export const resolveStoredIssueAttachmentPath = (storagePath: string) => {
  const resolvedTargetPath = path.isAbsolute(storagePath)
    ? storagePath
    : path.join(process.cwd(), storagePath);
  const normalizedTargetPath = path.resolve(resolvedTargetPath);
  const normalizedAllowedRoots = [resolvedUploadRoot, resolvedReceiptUploadRoot].map(
    (uploadRoot) => path.resolve(uploadRoot)
  );

  const isWithinAllowedRoot = normalizedAllowedRoots.some(
    (uploadRoot) =>
      normalizedTargetPath === uploadRoot ||
      normalizedTargetPath.startsWith(`${uploadRoot}${path.sep}`)
  );
